                    ...
                    for feature in class_feature:  
                        if len(feature_mean[i])==0:                      
                            ...
                            for j in range(len(feature_mean1)):
                                dis += torch.abs(torch.tensor(feature-feature_mean1[j].detach()))  
                            p_loss = torch.mean(1/dis) 
                            feature_false_loss += 0.1 * p_loss               
                        else:
                            dis = torch.tensor(feature-feature_mean[i].detach())
                            p_loss = torch.mean(torch.abs(dis))
...
def calculate_mean_vector(feat_cls, outputs, labels=None):
    ...
    for n in range(feat_cls.size()[0]):
        ...
        for t in range(19):
            ...
            nonzerofeat = torch.abs(false_s1).sum(dim=0)>1e-4
            false_s2 = false_s1.sum(dim=1)/nonzerofeat.sum()
            # false_s3 = F.normalize(false_s2, p=2, dim=0)
            false_vectors.append(torch.unsqueeze(false_s2, 0))
            false_ids.append(t)
