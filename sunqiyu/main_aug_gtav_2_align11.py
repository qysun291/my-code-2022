...
def main():
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            all_false_vectors = false_vectors + false_t_vectors
            all_false_ids = false_ids + false_t_ids
            false_class_features = np.array(list(constractive(all_false_ids, all_false_vectors).values()))
            ...
            for i in range(len(class_features)):
                if len(class_features[i])==0:
                    ...
                    # mean = np.mean(class_features[i], axis=1).to(device)
                    mean = torch.mean(torch.tensor(class_features[i]), dim=1).to(device)
                    feature_mean.append(mean)
                    feature_mean1.append(mean)
                    p_loss = torch.mean((torch.tensor(class_features[i]).to(device)-mean)**2)
                    feature_loss += p_loss
                    continue                
                else:
                    class_feature = np.array(class_features[i])
                    class_feature_con = torch.tensor(np.concatenate(class_feature, 0))
                    mean = torch.mean(class_feature_con, dim=0).to(device)
                    # class_feature = nn.functional.normalize(class_feature, p=2, dim=1)
                    feature_mean.append(mean)
                    feature_mean1.append(mean)
                    for feature in class_feature:
                        p_loss = torch.mean((torch.tensor(feature).to(device)-mean)**2)
                        feature_loss += p_loss
                # else:
                #     class_feature = torch.tensor(np.array(class_features[i]))
                #     # class_feature = nn.functional.normalize(class_feature, p=2, dim=1)
                #     for feature in class_feature:
                #         value = (feature-torch.tensor(feature_mean[i]))
                #         # value = torch.where(feature==0.0, torch.FloatTensor([0.0]), value)
                #         feature_false_loss += torch.mean(value**2)   
            ...
            for i in range(len(feature_mean1)):            
                ...
                for j in range(len(feature_mean1)-i-1):
                    ...
                    p_loss = torch.mean((a-b)**2) 
                    constractive_loss +=  p_loss              
            # constractive_loss = constractive_loss/256
            ...
            for i in range(len(false_class_features)):
                if len(false_class_features[i])==0:
                    ...
                    # class_feature = nn.functional.normalize(class_feature, p=2, dim=1)
                    for feature in class_feature:
                        value = (torch.tensor(feature).to(device)-feature_mean[i])
                        # value = torch.where(feature==0.0, torch.FloatTensor([0.0]), value)
                        p_loss = torch.mean(value**2)
                        feature_false_loss += p_loss                
            # feature_false_loss = feature_false_loss/256
                                    
            loss = (0.5 * (loss_seg_out + loss_seg_trans)  + feature_loss + feature_false_loss)-constractive_loss
            ...
            if (cur_itrs) % 10 == 0:
                ...
                print("Epoch %d, Itrs %d/%d, Loss=%f,  Feature_loss=%f, False_feature_loss=%f, Cons_loss=%f" %
                      (cur_epochs, cur_itrs, opts.total_itrs, interval_loss,  interval_feat_loss, interval_false_loss, interval_cons_loss))
                interval_loss = 0.0
...
def calculate_mean_vector(feat_cls, outputs, labels=None):
    ...
    for n in range(feat_cls.size()[0]):
        for t in range(19):
            ...
            vectors.append(s2)
            # original_vectors.append(original_s)
            ids.append(t)
            ...
            # false_s = false_s[0:10,:]
            # false_s = F.adaptive_avg_pool1d(false_s, 1000) / false_scale_factor[n][t]
            false_vectors.append(false_s2)
            false_ids.append(t)
