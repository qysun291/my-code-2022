...
def main():
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            loss_scr = 10 * torch.clamp((criterion_kl(nn.functional.log_softmax(trans_outputs, dim=1), nn.functional.softmax(outputs, dim=1)))/(torch.prod(torch.tensor(outputs.shape[1:]))), min=0)
                        
            loss = 0.5 * (loss_seg_out + loss_seg_trans + loss_seg) + loss_scr
            loss.backward()
            optimizer.step()
