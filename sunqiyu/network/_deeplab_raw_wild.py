...
class DeepLabHeadV3Plus(nn.Module):
    ...
    def forward(self, feature):
        ...
        aux_out = self.dsn(feature['aux_out'])
        output_feature = self.aspp(feature['out'])
        
        output_feature = self.bot_aspp(output_feature)
        # low_level_feature = self.bot_fine(feature['low_level'])
        
        output_feature = F.interpolate(output_feature, size=low_level_feature.shape[2:], mode='bilinear', align_corners=False)
        all_feature = torch.cat( [ low_level_feature, output_feature ], dim=1 ) 
        dec1 = self.final1(all_feature)
