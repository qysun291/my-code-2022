...
def main():
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            outputs, f_out, f_low= model(images)
            trans_outputs,t_f_out, t_f_low = model(trans_images)
            ...
            # torch.norm(f_out-t_f_out, p=2)/f_out.numel()
            low_feature_loss = torch.mean((f_low-t_f_low)**2)
            # torch.norm(f_low-t_f_low, p=2)/f_low.numel()
            
            #feature_align
            # vectors, ids = calculate_mean_vector(feat.detach(), outputs.detach(), labels=labels)
            # class_vectors = update_objective_SingleVector(ids, vectors).to(device) #[19,256]
            # class_vectors = normalize(class_vectors)
                        
            loss = 0.5 * (loss_seg_out + loss_seg_trans + loss_seg) + out_feature_loss + low_feature_loss
            loss.backward()
            optimizer.step()

            np_loss = loss.detach().cpu().numpy()
            # np_cons_loss = loss_cons.detach().cpu().numpy()
