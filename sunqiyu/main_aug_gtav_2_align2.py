...
def main():
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            #feature_align
            # vectors, ids = calculate_mean_vector(f_out.detach(), outputs_raw.detach(), labels=labels)
            # class_vectors = update_objective_SingleVector(ids, vectors).to(device) #[19,256]
            # class_vectors = nn.functional.normalize(class_vectors, p=2, dim=1)
            
            # t_vectors, t_ids = calculate_mean_vector(t_f_out.detach(), trans_outputs_raw.detach(), labels=labels)
            # t_class_vectors = update_objective_SingleVector(t_ids, t_vectors).to(device) #[19,256]
            # t_class_vectors = nn.functional.normalize(t_class_vectors, p=2, dim=1)
