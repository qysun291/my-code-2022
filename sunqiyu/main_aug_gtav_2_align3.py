...
def main():
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            #feature_align
            vectors, ids = calculate_mean_vector(f_out.detach(),  labels=labels)
            class_vectors = update_objective_SingleVector(ids, vectors).to(device) #[19,256]
            class_vectors = nn.functional.normalize(class_vectors, p=2, dim=1)
            
            t_vectors, t_ids = calculate_mean_vector(t_f_out.detach(),  labels=labels)
            ...
            #feature_align
            low_vectors, low_ids = calculate_mean_vector(f_low.detach(),  labels=labels)
            ...
            low_t_vectors, low_t_ids = calculate_mean_vector(t_f_low.detach(),  labels=labels)
            ...
            loss = 0.5 * (loss_seg_out + loss_seg_trans + loss_seg) + (feature_loss + low_feature_loss)
            loss.backward()
            optimizer.step()
...
def calculate_mean_vector(feat_cls, labels=None):
...
def update_objective_SingleVector(ids, vectors):
    ...
    objective_vectors = torch.zeros([19, vectors[0].size()[0]])
