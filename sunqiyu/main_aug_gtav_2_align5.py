...
def main():
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            # out_feature_loss = torch.mean((f_out-t_f_out)**2)
            # low_feature_loss = torch.mean((f_low-t_f_low)**2)

            # prob_out_o = nn.functional.log_softmax(f_out, dim=1)
            # prob_trans_o = nn.functional.softmax(t_f_out, dim=1)
            # loss_cons_o = 10 * nn.functional.kl_div(prob_out_o, prob_trans_o, reduction='mean')
            
            # prob_out_l = nn.functional.log_softmax(f_low, dim=1)
            # prob_trans_l = nn.functional.softmax(t_f_low, dim=1)
            # loss_cons_l = 10 * nn.functional.kl_div(prob_out_l, prob_trans_l, reduction='mean')
            ...
            feature_loss = 0.1 * torch.sum((class_vectors-t_class_vectors)**2) 
            feature_false_loss = 0.1 * (torch.sum((class_vectors-false_class_vectors)**2)+ torch.sum((t_class_vectors-false_t_class_vectors)**2))
            #feature_align
            low_vectors, low_ids, false_low_vectors, false_low_ids = calculate_mean_vector(f_low.detach(), outputs_raw, labels=labels)
            low_class_vectors = update_objective_SingleVector(low_ids,  low_vectors).to(device) #[19,256]
            low_class_vectors = nn.functional.normalize(low_class_vectors, p=2, dim=1)
            false_low_class_vectors = update_objective_SingleVector(false_low_ids,  false_low_vectors).to(device) #[19,256]
            false_low_class_vectors = nn.functional.normalize(false_low_class_vectors, p=2, dim=1)
            
            low_t_vectors, low_t_ids, false_low_t_vectors, false_low_t_ids = calculate_mean_vector(t_f_low.detach(), trans_outputs_raw, labels=labels)
            low_t_class_vectors = update_objective_SingleVector(low_t_ids, low_t_vectors).to(device) #[19,256]
            low_t_class_vectors = nn.functional.normalize(low_t_class_vectors, p=2, dim=1)
            false_low_t_class_vectors = update_objective_SingleVector(false_low_t_ids, false_low_t_vectors).to(device) #[19,256]
            false_low_t_class_vectors = nn.functional.normalize(false_low_t_class_vectors, p=2, dim=1)
            
            low_feature_loss = 0.1 * torch.sum((low_class_vectors-low_t_class_vectors)**2)
            low_feature_false_loss = 0.1 * (torch.sum((low_class_vectors-false_low_class_vectors)**2)+torch.sum((low_t_class_vectors-false_low_t_class_vectors)**2))
            #end
            
                                    
            loss = 0.5 * (loss_seg_out + loss_seg_trans + loss_seg) + (feature_loss + low_feature_loss) + (feature_false_loss + low_feature_false_loss)
            loss.backward()
            optimizer.step()

            np_loss = loss.detach().cpu().numpy()
            np_cons_loss = (feature_loss + low_feature_loss).detach().cpu().numpy()
            ...
            if (cur_itrs) % 10 == 0:
                ...
                print("Epoch %d, Itrs %d/%d, Loss=%f, Feature_loss=%f, False_loss=%f" %
                      (cur_epochs, cur_itrs, opts.total_itrs, interval_loss, interval_cons_loss, interval_false_loss))
                interval_loss = 0.0
