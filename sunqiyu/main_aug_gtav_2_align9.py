...
def main():
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            loss = (0.5 * (loss_seg_out + loss_seg_trans + loss_seg) + feature_loss + feature_false_loss)-constractive_loss
...
def calculate_mean_vector(feat_cls, outputs, labels=None):
    ...
    for n in range(feat_cls.size()[0]):
        for t in range(19):
            ...
            s = s.reshape(-1,256)
            nonzerofeat = torch.abs(s).sum(dim=1)>0
            s = s[nonzerofeat,:]           
            ...
            false_s = false_s.reshape(-1,256)
            nonzerofeat = torch.abs(false_s).sum(dim=1)>0
            false_s = false_s[nonzerofeat,:]
