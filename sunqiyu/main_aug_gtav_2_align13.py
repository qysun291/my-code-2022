...
def get_dataset(opts):
    ...
    # train_syn_dst = SYNTHIA(list_path='../DeepLabV3Plus-Pytorch-master/datasets/data/SYNTHIA/train.txt',transform=train_transform)
    # val_syn_dst = SYNTHIA(list_path='../DeepLabV3Plus-Pytorch-master/datasets/data/SYNTHIA/train.txt',transform=train_query_transform)

    train_gtav_dst = GTAV(list_path='../DeepLabV3Plus-Pytorch-master/datasets/data/GTAV/train.txt',transform=train_transform)

    return train_cs_dst, val_cs_dst, train_gtav_dst
...
def main():
    ...
    train_cs_dst, val_cs_dst, train_syn_dst = get_dataset(opts)
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            #feature_align
            vectors,  ids, false_vectors, false_ids = calculate_mean_vector(f_out.detach(), outputs,  labels=labels)            
            t_vectors,  t_ids, false_t_vectors, false_t_ids = calculate_mean_vector(t_f_out.detach(), trans_outputs, labels=labels)

            #true
            all_vectors = torch.cat((vectors, t_vectors), 0)
            all_vectors = nn.functional.normalize(all_vectors, p=2, dim=1)
            all_false_vectors = torch.cat((false_vectors, false_t_vectors), 0)
            all_false_vectors = nn.functional.normalize(all_false_vectors, p=2, dim=2)
            # num = all_vectors.size()[0]
            # 
            
            # vectors = torch.cat((all_vectors, all_false_vectors), 0)
            # vectors = nn.functional.normalize(vectors, p=2, dim=0)
            
            # all_vectors = vectors[:num,:]
            # all_false_vectors = vectors[num:,:]
            #false
            
            # all_false_vectors = nn.functional.normalize(all_false_vectors, p=2, dim=1)
            
            all_ids = ids + t_ids
            class_features = np.array(constractive(all_ids, all_vectors))
            
            all_false_ids = false_ids + false_t_ids
            false_class_features = np.array(constractive(all_false_ids, all_false_vectors))
            
            feature_loss = 0
            feature_false_loss = 0
            feature_mean = []
            feature_mean1 = []
            for i in range(len(class_features)):
                if len(class_features[i])==0:
                    feature_mean.append([])
                    continue
                elif len(class_features[i])==1:    
                    mean = class_features[i][0]
                    feature_mean.append(mean)
                    feature_mean1.append(mean)
                    continue                
                else:
                    mean = np.mean(class_features[i], axis=0)
                    feature_mean.append(mean)
                    feature_mean1.append(mean)
                    for feature in class_features[i]:
                        # dis = torch.tensor(feature-mean)
                        # p_loss = torch.mean(torch.abs(dis))
                        false_feat = nn.functional.log_softmax(torch.tensor(feature), dim=0)
                        true_feat = nn.functional.softmax(torch.tensor(mean), dim=0).unsqueeze(1)
                        p_loss = 10 * nn.functional.kl_div(false_feat, true_feat.detach(), reduction='batchmean')                            

                        feature_loss += p_loss
 

            # constractive_loss = 0.0 
            # feature_mean1 = nn.functional.normalize(torch.tensor(feature_mean1), p=2, dim=1)           
            # for i in range(len(feature_mean1)):            
            #     a = feature_mean1[i]
            #     for j in range(len(feature_mean1)-i-1):
            #         b = feature_mean1[i+j+1]
            #         dis =  torch.tensor(np.array(a)-np.array(b))
            #         p_loss = torch.mean(torch.abs(dis)) 
            #         constractive_loss +=  p_loss              
            
            
            # feature_gram_matrix = gram_matrix(torch.tensor(feature_mean1))
            
            for i in range(len(false_class_features)):
                if len(false_class_features[i])==0:
                    continue              
                else:
                    class_feature = np.array(false_class_features[i])
                    for feature in class_feature:  
                        if len(feature_mean[i])==0:                      
                            dis = 0.0
                            for j in range(len(feature_mean1)):
                            #     dis += torch.abs(torch.tensor(feature-feature_mean1[j]))  
                            # p_loss = torch.mean(1/dis) 
                                false_feat = nn.functional.log_softmax(torch.tensor(feature), dim=0)
                                true_feat = nn.functional.softmax(torch.tensor(feature_mean1[j]), dim=0).unsqueeze(1)
                                dis += 10 * nn.functional.kl_div(false_feat, true_feat.detach(), reduction='batchmean') 
                            p_loss = 1/dis                                                                                  
                            feature_false_loss += 0.1 * p_loss               
                        else:
                            # dis = torch.tensor(feature-feature_mean[i])
                            # p_loss = torch.mean(torch.abs(dis))
                            false_feat = nn.functional.log_softmax(torch.tensor(feature), dim=0)
                            true_feat = nn.functional.softmax(torch.tensor(feature_mean[i]), dim=0).unsqueeze(1)
                            p_loss = 10 * nn.functional.kl_div(false_feat, true_feat.detach(), reduction='batchmean')                            
                            feature_false_loss += p_loss 

                                    
            loss = (0.5 * (loss_seg_out + loss_seg_trans)  + feature_loss + feature_false_loss)
            # /constractive_loss
            ...
            np_feat_loss = feature_loss.detach().cpu().numpy()
            np_false_loss = feature_false_loss.detach().cpu().numpy()
            # np_cons_loss = constractive_loss.detach().cpu().numpy()
            
            interval_loss += np_loss
            interval_feat_loss += np_feat_loss
            ...
            if (cur_itrs) % 10 == 0:
                ...
                interval_feat_loss = interval_feat_loss / 10
                interval_false_loss = interval_false_loss / 10
                # interval_cons_loss = interval_cons_loss/10
                print("Epoch %d, Itrs %d/%d, Loss=%f,  Feature_loss=%f, False_feature_loss=%f" %
                      (cur_epochs, cur_itrs, opts.total_itrs, interval_loss,  interval_feat_loss, interval_false_loss))
                interval_loss = 0.0
                interval_feat_loss = 0.0
...
def transfer_lab(src):
    ...
    for j in range(len(src)):
        ...
        for i in range(3):
            ...
            img[:,:,i] = ((img[:,:,i]-mean)/(std+1e-5)) * t_std[i] + t_mean[i]
            # np.clip(img, 0, 255)
...
def calculate_mean_vector(feat_cls, outputs, labels=None):
    ...
    for n in range(feat_cls.size()[0]):
        for t in range(19):
            ...
            s1 = s.reshape(256,-1)
            nonzerofeat = torch.abs(s1).sum(dim=0)>1e-4
            s2 = s1.sum(dim=1)/nonzerofeat.sum()
            # s3 = F.normalize(s2, p=2, dim=0)       
            vectors.append(torch.unsqueeze(s2, 0))
            ids.append(t)
        ...
        for t in range(19):
            ...
            false_s1 = false_s.reshape(256,-1)
            nonzerofeat = torch.abs(false_s1).sum(dim=0)
            nonzeroindex = torch.nonzero(nonzerofeat>1e-4).squeeze()
            false_s2 = false_s1[:,nonzeroindex]
            choose_index = torch.randint(0, false_s2.size()[1], (1, 600)).squeeze()
            false_s3 = false_s2[:,choose_index]

            false_vectors.append(torch.unsqueeze(false_s3, 0))
            false_ids.append(t)
    cat_vectors = torch.cat(vectors, 0)
    # cat_vectors = nn.functional.normalize(cat_vectors, p=2, dim=1)
    cat_false_vectors = torch.cat(false_vectors, 0)
    # cat_false_vectors = nn.functional.normalize(cat_false_vectors, p=2, dim=1)
    return cat_vectors, ids, cat_false_vectors, false_ids
...
def constractive(ids, vectors):
    ...
    return_feature = list(class_feature.values())
