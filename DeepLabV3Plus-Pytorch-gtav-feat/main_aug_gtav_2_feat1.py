...
def get_dataset(opts):
    ...
    train_transform = et.ExtCompose([
        # et.ExtResize( 512 ),
        et.ExtRandomScale(scale_range=[0.53, 2.03]),
...
def main():
    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            outputs = model(images)
            trans_outputs, x_low = model(trans_images)
            trans_outputs_feat, _ = model(images, x_low)
            outputs_max, id = torch.max(torch.cat((outputs.unsqueeze(0),trans_outputs.unsqueeze(0), trans_outputs_feat.unsqueeze(0)),0),0) 
            
            loss_seg_out = criterion(outputs, labels)
            loss_seg_trans = criterion(trans_outputs, labels)
            loss_seg_feat = criterion(trans_outputs_feat, labels)
            loss_seg = criterion(outputs_max, labels)
            
            # criterion_kl = nn.KLDivLoss(reduction='batchmean').cuda()
            
            # loss_scr = torch.clamp((criterion_kl(nn.functional.log_softmax(trans_outputs, dim=1), nn.functional.softmax(outputs, dim=1)))/(torch.prod(torch.tensor(outputs.shape[1:]))), min=0)
                        
            loss = 0.5 * (loss_seg_out + loss_seg_trans + loss_seg + loss_seg_feat)
            loss.backward()
            optimizer.step()

            np_loss = loss.detach().cpu().numpy()
            np_cons_loss = 0.5 * loss_seg_feat.detach().cpu().numpy()
            ...
            if (cur_itrs) % 10 == 0:
                ...
                print("Epoch %d, Itrs %d/%d, Loss=%f, Con_loss=%f" %
