...
def x_adain(x_cont):
    ...
    content_mean, content_std, style_mean, style_std = calc_mean_std(x_cont)
    ...
    style_mean = style_mean.clone().detach()[index]     
    style_std = style_std.clone().detach()[index] 

    # if random.random() < 0.5:
    #     style_mean = style_mean * ((torch.rand(size_mean).to(torch.device('cuda')) - 0.5)  + 1)
    #     style_std = style_std * ((torch.rand(size_mean).to(torch.device('cuda'))- 0.5)  + 1)
    
    # if random.random() < 0.5:
    #     content_mean = content_mean * ((torch.rand(size_mean).to(torch.device('cuda')) - 0.5)  + 1)
    #     content_std = content_std * ((torch.rand(size_mean).to(torch.device('cuda')) - 0.5)  + 1)
...
def calc_mean_std(feat, eps=1e-5):
    ...
    #style  
    # h_crop_size = 0.5 * size[2] 
    # w_crop_size = 0.5 * size[3] 
    h = random.randint(0.75 * size[2], size[2])
    w = random.randint(0.75 * size[3], size[3])        
    h_start = random.randint(0, size[2] - h)
    w_start = random.randint(0, size[3] - w)
    crop_feat = feat[:, :, h_start: h_start+h, w_start: w_start+w]    

    crop_size = crop_feat.size()    
    N_crop, C_crop = crop_size[:2]
    style_feat_var = crop_feat.contiguous().view(N_crop, C_crop, -1).var(dim=2) + eps
    style_feat_std = style_feat_var.sqrt().contiguous().view(N_crop, C_crop, 1, 1)
    style_feat_mean = crop_feat.contiguous().view(N_crop, C_crop, -1).mean(dim=2).contiguous().view(N_crop, C_crop, 1, 1)
    
    return feat_mean, feat_std, style_feat_mean, style_feat_std